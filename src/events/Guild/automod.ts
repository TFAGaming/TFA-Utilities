...
// Every possible swear word and it's bypasses in the entire world tbh
...
export const _mainAutomodFunction = (message: Message) => {
    if (message.author.bot) return;
    if (!message.guild) return;
    if (!message.content) return;

    if (protectedRolesChecker(message.member, config.automod.protectedRoles)) return;
...
export default new Event('messageCreate', _mainAutomodFunction);
